    ...
    try:
        ...
        await send_notification_to_user(
            str(project.developer_id),
            {
                ...
                "message": f"Payment confirmed! Chat with investor about '{project.title[:50]}' is now active.",
        ...
        await send_notification_to_user(
            str(investor_id),
            {
                "type": "chat_room_created",
                "title": "💬 Chat Room Ready",
                "message": f"Payment confirmed! Chat with developer about '{project.title[:50]}' is now active.",
                "link": f"/chat/{chat_room.id}",
                "details": {
                    "project_id": project_id,
                    "room_id": str(chat_room.id)
