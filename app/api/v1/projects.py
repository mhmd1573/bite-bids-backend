...
#         # ✅ GET ALL ADMIN USERS ONCE
#         admin_users_query = await db.execute(
#             select(User).where(User.role == 'admin')
#         )
#         admin_users = admin_users_query.scalars().all()
#         logger.info(f"Found {len(admin_users)} admin users for notifications")
...
#             # ✅ FIXED: Create notification for EACH admin
#             for admin in admin_users:
#                 admin_notification = Notification(
#                     user_id=admin.id,  # ✅ Use actual admin ID
#                     type='payout_pending',
...
async def simple_approve_project(
    ...
    """
    Simplified project approval - investor confirms completion
    ✅ UPDATED: Handles multiple investors per project
    ✅ UPDATED: Uses manual bank transfer for payouts
    """
    try:
        # Get project
        ...
        # ✅ Check if THIS investor already has a payout for this project
        existing_payout_query = await db.execute(
            select(DeveloperPayout).where(
                and_(
                    DeveloperPayout.project_id == project.id,
                    DeveloperPayout.investor_id == user_uuid
                )
            )
        )
        existing_payout = existing_payout_query.scalar_one_or_none()
        ...
        if existing_payout:
            ...
            return {
                ...
                "project_id": str(project.id),
                "project_status": project.status,
                "developer_payout": float(existing_payout.net_amount),
        ...
        # Update project status (only if not already completed)
        if project.status != 'completed':
            project.status = 'completed'
            project.completed_at = datetime.utcnow()
            logger.info(f"Project {project_id} marked as completed (was {original_status})")
        else:
            logger.info(f"Project {project_id} already completed")
        ...
        if not has_bank_details:
            ...
            for admin in admin_users:
                admin_notification = Notification(
                    ...
                    type='payout_pending',
                    title='💰 Payout Pending - Bank Details Needed',
                    message=f'Developer {developer.name} needs to add bank details to receive ${developer_payout:.2f} for project "{project.title}" from investor {current_user["name"]}.',
                    link='/admin/payouts',
                    details={
                        'project_id': str(project.id),
                        'project_title': project.title,
        ...
        dev_notification = Notification(
            ...
            type='project_approved',
            title='🎉 Project Approved!',
            message=f'Your project "{project.title}" has been approved by investor {current_user["name"]}. Payment of ${developer_payout:.2f} is pending and will be processed via bank transfer.',
            link=f"/projects/{project_id}",
            details={
                'project_id': str(project.id),
                'project_title': project.title,
        ...
        for admin in admin_users:
            admin_notification = Notification(
                ...
                type='payout_pending',
                title='💰 New Payout Pending',
                message=f'Developer {developer.name} has a pending payout of ${developer_payout:.2f} for project "{project.title}" from investor {current_user["name"]}. Please process the bank transfer.',
                link='/admin/payouts',
                details={
                    'project_id': str(project.id),
                    'project_title': project.title,
